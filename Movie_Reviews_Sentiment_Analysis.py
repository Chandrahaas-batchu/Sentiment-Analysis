...
train = pd.read_csv('E:/Github/Sentiment_Analysis/Movie_Review_Sentimentals/train.tsv/train.tsv', sep="\t")
test = pd.read_csv('E:/Github/Sentiment_Analysis/Movie_Review_Sentimentals/test.tsv/test.tsv', sep="\t")
sub = pd.read_csv('E:/Github/Sentiment_Analysis/Movie_Review_Sentimentals/sampleSubmission.csv', sep=",")
...
1.using stopwords can be a bad idea, especially when phrases contain one single stopword;
2.puntuation could be important, so it should be used;
3.ngrams are necessary to get the most info from data;
4.using features like word count or sentence length won't be useful;
# In[ ]:
...
# ### Deep learning
# And now let's try DL. DL should work better for text classification with multiple layers. 
